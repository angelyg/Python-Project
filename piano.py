# Imports the wavfile from the library SciPy.
from scipy.io import wavfile

# Imports argparse; Used for updating names and classes that are used within the code.
import argparse

# Imports the library Numpy.
import numpy as np
...
# Imports Warning category classes; subclass to Exception.
import warnings

# speed function
def speedx(snd_array, factor):
    """ Speeds up / slows down a sound"""
...
# Stretches/shortens a sound
    ...
    for i in np.arange(0, len(snd_array) - (window_size + h), h*factor):
        ...
        # The spectra (SPY) of these arrays.
        s1 = np.fft.fft(hanning_window * a1)
        s2 = np.fft.fft(hanning_window * a2)

        # Rephase all frequencies used.
...
def main():
    ...
    # Enable warnings from Scipy if necessary.
    if not args.verbose:
    ...
    while True:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # Stops with 50ms fadeout; pause in between key press
